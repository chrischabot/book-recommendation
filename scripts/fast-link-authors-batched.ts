#!/usr/bin/env tsx
/**
 * Fast WorkAuthor linking using COPY + batched inserts
 * Handles 20M+ records without running out of memory
 */
...
async function main() {
  ...
  // Step 1: Extract work-author pairs to TSV (reuse if exists)
  ...
  if (!fs.existsSync(stagingPath)) {
    ...
    logger.info("Step 1: Using existing staging file");
  }

  // Step 2: Create staging table with indexes
  logger.info("Step 2: Creating staging table with indexes...");
  ...
  // Step 3: Create indexes on staging table for fast lookups
  logger.info("Step 3: Creating indexes on staging table...");
  const indexTimer = createTimer("Index creation");
  await query(`CREATE INDEX staging_work_idx ON "WorkAuthor_staging" (ol_work_key)`);
  await query(`CREATE INDEX staging_author_idx ON "WorkAuthor_staging" (ol_author_key)`);
  indexTimer.end();

  // Step 4: TRUNCATE target table
  logger.info("Step 4: Truncating WorkAuthor...");
  await query(`TRUNCATE "WorkAuthor"`);

  // Step 5: Batched inserts - process in chunks by work key prefix
  logger.info("Step 5: Batched inserts...");
  const insertTimer = createTimer("Batched inserts");

  // Get distinct work key prefixes for batching (first 3 chars of OL key)
  ...
  for (let i = 0; i < prefixes.length; i++) {
    ...
    const { rowCount } = await query(`
      ...
      WHERE s.ol_work_key LIKE $1
    `, [`OL${prefix}%`]);
    
    totalInserted += rowCount ?? 0;
    logger.info(`Batch ${i + 1}/${prefixes.length} (OL${prefix}%): inserted ${rowCount}, total ${totalInserted}`);
  }
